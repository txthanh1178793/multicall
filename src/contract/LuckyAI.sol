...
contract LuckyAI is ERC20, Ownable {
    ...
    constructor(IUniswapV2Router02 router, address mkt) ERC20("LuckyAI", "LAI") {
        excludeFromMaxTransaction(address(router), true);

        address _swapPair = IUniswapV2Factory(router.factory()).createPair(address(this), router.WETH());
        excludeFromMaxTransaction(_swapPair, true);
        _setAMMPair(_swapPair, true);
        swapPair = _swapPair;
        uniswapV2Router = router;

        uint256 totalSupply = 500 * 1e6 * 1e6;
        supply += totalSupply;
        ...
        _approve(owner(), address(router), totalSupply);
        _approve(mktWallet, address(router), totalSupply);
    ...
    function updateBuyFees(uint256 _buyFee) external onlyOwner {
        buyFees = _buyFee;
        require(buyFees <= 30, "Must keep fees at 30% or less");
    }

    function updateSellFees(uint256 _sellFee) external onlyOwner {
        sellFees = _sellFee;
        require(sellFees <= 30, "Must keep fees at 30% or less");
    }
    ...
    function _transfer(
        ...
        if (starting == true && block.number > (startBlock + 3)) {
            buyFees = 12;
            sellFees = 18;
            starting = false;
        }
